...
{
    class SecretKeyCrypto : NativeResource
    {
        ...
        public byte[] Encrypt(byte[] message)
        {
            ...
            var err = SafeNativeMethods.CryptoSecretboxEasy(cipherTextPtr, messagePtr, message.Length, _noncePtr, _keyPtr);
        ...
        public byte[] Decrypt(byte[] cipherText, byte[] nonce, byte[] key)
        {
            ...
            var err = SafeNativeMethods.CryptoSecretboxOpenEasy(decryptedPtr, cipherTextPtr, cipherText.Length, _noncePtr, _keyPtr);
            if (err != 0) { throw new Exception(); }
